...
public class BossSlime extends EventBase
{
	private Location _loc;

	public BossSlime(EventManager manager) 
	{
		super(manager, "Slime King", 1); 
	}
	...
	private void FindLocation() 
	{
		...
		if (_loc != null)
		{
			Location loc = Manager.TerrainFinder().LocateSpace(_loc, 8, 0, 5, 0, false, false, GetBlocks().keySet());
			...
			CreatureRegister(new SlimeKing(this, loc.add(0.5, 0.5, 0.5)));
		}
	}
	...
	{
		UtilServer.broadcast(F.main("World Event", F.elem("Slime King") + " has entered the world."));	
	...
	{
		UtilServer.broadcast(F.main("World Event", F.elem("Slime King") + " has been defeated!"));
	...
	{
		UtilServer.broadcast(F.main("World Event", F.elem("Slime King") + " has left the world."));		
